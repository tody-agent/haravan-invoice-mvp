...
export default function ReportDeleted() {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    fetch(`${API_URL}/v1/reports/deleted?dateFrom=${dateFrom}&dateTo=${dateTo}`, {
  ...
  return (
    <>
      <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--hv-space-3)', marginBottom: 'var(--hv-space-4)' }}>
        ...
        <h2 className="text-h2">Hóa đơn xóa bỏ</h2>
      </div>

      <div className="hv-card" style={{ marginBottom: 'var(--hv-space-4)', background: 'var(--hv-danger-bg, #fef2f2)', border: '1px solid var(--hv-danger-border, #fecaca)' }}>
        <div style={{ display: 'flex', alignItems: 'flex-start', gap: 'var(--hv-space-3)' }}>
          <i className="ti ti-alert-triangle" style={{ color: 'var(--hv-danger)', fontSize: 20, marginTop: 2 }}></i>
          <div>
            <div className="text-body-strong" style={{ color: 'var(--hv-danger)', marginBottom: 'var(--hv-space-1)' }}>
              Lưu ý theo Nghị định 70/2025/NĐ-CP
            </div>
            <p className="text-caption" style={{ margin: 0 }}>
              Hóa đơn xóa bỏ phải được lưu trữ tối thiểu 10 năm theo quy định. Báo cáo này phục vụ mục đích tra cứu và đối chiếu với cơ quan thuế.
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="hv-card">
        {loading ? (
          ...
          <div className="hv-empty-state" style={{ display: 'flex' }}>
            <i className="ti ti-trash hv-empty-state-icon" aria-hidden="true"></i>
            <div className="hv-empty-state-title">Không có hóa đơn xóa bỏ</div>
              ...
              <tbody>
                {data.map((item: any, i: number) => (
                  <tr key={item.id || i}>
                    ...
                    <td>{formatDate(item.deletedDate || item.updatedAt)}</td>
                    ...
                    <td><span className="text-caption">{item.reason || '-'}</span></td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        {data.length > 0 && (
          <div style={{ padding: 'var(--hv-space-3) var(--hv-space-4)', borderTop: '1px solid var(--hv-border)' }}>
            <span className="text-caption">Tổng: {data.length} hóa đơn xóa bỏ</span>
          </div>
        )}
      </div>
    </>
