...
export default function ReportReplaced() {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    fetch(`${API_URL}/v1/reports/replaced?dateFrom=${dateFrom}&dateTo=${dateTo}`, {
  ...
  return (
    <>
      <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--hv-space-3)', marginBottom: 'var(--hv-space-4)' }}>
        ...
        <h2 className="text-h2">Hóa đơn thay thế</h2>
      </div>
      ...
      <div className="hv-card">
        {loading ? (
          ...
          <div className="hv-empty-state" style={{ display: 'flex' }}>
            <i className="ti ti-refresh hv-empty-state-icon" aria-hidden="true"></i>
            <div className="hv-empty-state-title">Không có hóa đơn thay thế</div>
              ...
              <tbody>
                {data.map((item: any, i: number) => (
                  <tr key={item.id || i}>
                    ...
                    <td><span className="text-mono">{item.replacedInvoiceNo || item.replacedHaravanId || '-'}</span></td>
                    <td>{formatDate(item.replacedDate || item.updatedAt)}</td>
        ...
        {data.length > 0 && (
          <div style={{ padding: 'var(--hv-space-3) var(--hv-space-4)', borderTop: '1px solid var(--hv-border)' }}>
            <span className="text-caption">Tổng: {data.length} hóa đơn thay thế</span>
          </div>
        )}
      </div>
    </>
